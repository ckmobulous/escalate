...
public class TakeImage extends Activity
{
	...
	private final String capture_dir = Environment.getExternalStorageDirectory() + "/escalate/Images/";
	Activity activity;
...
//				imageFileUri = FileProvider.getUriForFile(TakeImage.this,BuildConfig.APPLICATION_ID + ".provider",new File(path));
				imageFileUri = FileProvider.getUriForFile(TakeImage.this,"com.escalate.provider",new File(path));
...
//	compressing image
	private String compressImage(Bitmap finalBitmap)
	{
...
//			ThumbNailBitmap bm = BitmapFactory.decodeFile(path, options);
			...
			if (requestCode == REQ_CODE_PICK_IMAGE)
			{
				try
				{
					...
					if (selectedImage != null)
					{
						...
						String finalpath=compressImage(bm);
						bm.recycle();
